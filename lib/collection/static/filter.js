...
module.exports = function (r, Promise, utils, errors, extend, models, collections, CollectionClass) {
	...
	function _filter(predicate, options) {
		...
		if (Collection.model) {
			...
			Model = Collection;
			Collection = extend.apply(CollectionClass, [
				{},
				{
					model: Model,
 ...
 *  });
	 ...
	 * - `{object}` - `collection` - If no error occurs, an instance of this Collection filled with instances of this
	 * Collection's Model.
	...
	function filter(predicate, options, cb) {
		...
		return Promise.resolve().then(function sanitize() {
			...
			return _filter.apply(Collection, [predicate, options, cb]);
		}).nodeify(cb);
	}
