module.exports = function (utils, errors, Promise) {
	...
	var protoProps = {
		...
		destroy: function (options, cb) {
			...
			return Promise.all(tasks).nodeify(cb);
		},
		 ...
		 * @name removeByPrimaryKey(primaryKey)
		 * @description
		 * Return the model instance with the given primary key if it exists in this collection.
		 *
		 * ## Signature:
	 ...
	 * @name findLast(callback[, thisObj])
	 * @description
	 ...
	 * @name findIndex(iterator[, thisObj])
	 * @description
	 ...
	 * @name findLastIndex(iterator[, thisObj])
	 * @description
