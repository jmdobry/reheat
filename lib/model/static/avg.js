...
/**
 * @method Model.avg
 * @desc Average aggregator method. Works on a single field or in conjunction with `groupBy`.
 * @static
 ...
 * @see http://rethinkdb.com/api/javascript/#avg
 */
module.exports = function avg(predicate, options, cb) {
	predicate = predicate || {};
	options = options || {};
	if (utils.isFunction(predicate)) {
		...
		throw new Error('Model.avg([predicate][, options], cb): cb: Must be a function!');
	} else if (!utils.isObject(predicate)) {
		cb(new Error('Model.avg([predicate][, options], cb): predicate: Must be an object!'));
	} else if (!utils.isObject(options)) {
		cb(new Error('Model.avg([predicate][, options], cb): options: Must be an object!'));
	} else if (!utils.isString(predicate.avg)) {
		cb(new Error('Model.avg([predicate][, options], cb): predicate.avg: Must be a string!'));
	}
