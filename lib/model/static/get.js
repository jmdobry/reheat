			...
			if (Model.relations.hasMany) {
				utils.forOwn(Model.relations.hasMany, function (relation, modelName) {
					if (!models[modelName]) {
						...
						newModels[localField] = modelName;
					}
				});
			}
				...
				if (!options.raw) {
					utils.forOwn(doc, function (localValue, localKey) {
						if (localKey in newModels) {
							if (utils.isString(localValue)) {
								doc[localKey] = new models[newModels[localKey]](doc[localKey]);
							} else if (utils.isObject(localValue)) {
								// TODO: Handle nested relation
							} else if (utils.isArray(localValue)) {
								for (var i = 0; i < localValue.length; i++) {
									localValue[i] = new models[newModels[localKey]](localValue[i]);
								}
							} else {
								// TODO: Error
							}
