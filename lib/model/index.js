 ...
 * ```
 *
 * See [Model.extend](/documentation/api/api/Model.static_methods:extend).
 */
 ...
 * - `{object=}`        - schema - Object that implements the necessary schema interface.
 */
Model.extend = extend;
...
meld.after(Model, 'extend', function (ChildModel) {
	...
	} else if (ChildModel.schema && !utils.isObject(ChildModel.schema)) {
		throw new errors.IllegalArgumentError(errorPrefix + 'staticProps.schema: Must be an object!', { schema: { actual: typeof ChildModel.schema, expected: 'object' } });
	} else if (ChildModel.schema && !utils.isFunction(ChildModel.schema.validate)) {
		throw new errors.IllegalArgumentError(errorPrefix + 'staticProps.schema.validate: Must be a function!', { schema: { validate: { actual: typeof ChildModel.schema.validate, expected: 'function' } } });
	}
});
...
meld.around(Model.prototype, 'save', function (jp) {
	jp.proceed(function (err, instance) {
		if (err) {
			jp.args[jp.args.length - 1](err);
		} else {
			// TODO: Test this method selection
			var method = !jp.target.previousAttributes || utils.isEmpty(jp.target.previousAttributes) || jp.target.meta.inserted ? jp.target.afterCreate : jp.target.afterUpdate;
			method.apply(jp.target, [instance, jp.args[jp.args.length - 1]]);
		}
	});
});
...
meld.around(Model.prototype, 'destroy', function (jp) {
	jp.proceed(function (err, instance) {
		if (err) {
			...
			jp.target.afterDestroy.apply(jp.target, [instance, jp.args[jp.args.length - 1]]);
		}
	});
});
