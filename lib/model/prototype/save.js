...
module.exports = function (r, Promise, utils, errors, models) {
	 ...
	 *
	 * - `{ValidationError}` - `err` - `null` if no error occurs.
	...
	function save(options, cb) {
		...
		return Promise.resolve().bind(this)
			...
			.then(function () {
				if (this.constructor.relations.belongsTo) {
					utils.forOwn(this.constructor.relations.belongsTo, function (relation, modelName) {
						if (!models[modelName]) {
							throw new errors.RuntimeError(this.constructor.name + ' Model defined belongsTo relationship to nonexistent ' + modelName + ' Model!');
						}
				...
				if (this.constructor.relations.hasMany) {
					utils.forOwn(this.constructor.relations.hasMany, function (relation, modelName) {
						if (!models[modelName]) {
							throw new errors.RuntimeError(this.constructor.name + ' Model defined hasMany relationship to nonexistent ' + modelName + ' Model!');
						}
				...
				if (this.constructor.relations.hasOne) {
					utils.forOwn(this.constructor.relations.hasOne, function (relation, modelName) {
						if (!models[modelName]) {
							throw new errors.RuntimeError(this.constructor.name + ' Model defined hasOne relationship to nonexistent ' + modelName + ' Model!');
						}
				...
				if (this.isNew() && this.constructor.schema && this.constructor.schema.defaults) {
					this.constructor.schema.addDefaultsToTarget(this.attributes);
				}
			...
			.then(function () {
				return this.isNew() ? Promise.promisify(this.beforeCreate, this)().bind(this).return(this.constructor) : Promise.promisify(this.beforeUpdate, this)().bind(this).return(this.constructor);
			})
			.then(function (Model) {
			...
			.then(function (Model) {
				if (Model.timestamps) {
					utils.deepMixIn(this.attributes, { deleted: null, updated: r.now(), created: r.now() });
				}
				return Model.connection.run(r.table(Model.tableName).insert(this.attributes, { return_vals: true, upsert: true }));
			})
						...
						utils.forOwn(childRelations, function (child) {
							if (child.isCollection) {
								child.value.forEach(function (item) {
									item.setSync(child.foreignKey, this.get(this.constructor.idAttribute));
								}, this);
							} else {
								child.value.setSync(child.foreignKey, this.get(this.constructor.idAttribute));
							}
