...
module.exports = function (r, Promise, gPool, utils, errors) {
	...
	function sanitizeConfigOptions(options) {
		if (!utils.isObject(options)) {
			throw new errors.IllegalArgumentError(configureErrorPrefix + 'options: Must be an object!', { actual: typeof options, expected: 'object' });
		} else if (options.port && !utils.isNumber(options.port)) {
			throw new errors.IllegalArgumentError(configureErrorPrefix + 'options.port: Must be a number!', { port: { actual: typeof options.port, expected: 'number' } });
		} else if (options.db && !utils.isString(options.db)) {
			throw new errors.IllegalArgumentError(configureErrorPrefix + 'options.db: Must be a string!', { db: { actual: typeof options.db, expected: 'string' } });
		} else if (options.host && !utils.isString(options.host)) {
			throw new errors.IllegalArgumentError(configureErrorPrefix + 'options.host: Must be a string!', { host: { actual: typeof options.host, expected: 'string' } });
		} else if (options.authKey && !utils.isString(options.authKey)) {
			throw new errors.IllegalArgumentError(configureErrorPrefix + 'options.authKey: Must be a string!', { authKey: { actual: typeof options.authKey, expected: 'string' } });
		} else if (options.name && !utils.isString(options.name)) {
			throw new errors.IllegalArgumentError(configureErrorPrefix + 'options.name: Must be a string!', { name: { actual: typeof options.name, expected: 'string' } });
		} else if (options.max && !utils.isNumber(options.max)) {
			throw new errors.IllegalArgumentError(configureErrorPrefix + 'options.max: Must be a number!', { max: { actual: typeof options.max, expected: 'number' } });
		} else if (options.min && !utils.isNumber(options.min)) {
			throw new errors.IllegalArgumentError(configureErrorPrefix + 'options.min: Must be a number!', { min: { actual: typeof options.min, expected: 'number' } });
		} else if (options.idleTimeoutMillis && !utils.isNumber(options.idleTimeoutMillis)) {
			throw new errors.IllegalArgumentError(configureErrorPrefix + 'options.idleTimeoutMillis: Must be a number!', { idleTimeoutMillis: { actual: typeof options.idleTimeoutMillis, expected: 'number' } });
		} else if (options.log && !utils.isBoolean(options.log) && !utils.isFunction(options.log)) {
			throw new errors.IllegalArgumentError(configureErrorPrefix + 'options.log: Must be a boolean or a function!', { log: { actual: typeof options.log, expected: 'boolean|function' } });
		} else if (options.refreshIdle && !utils.isBoolean(options.refreshIdle)) {
			throw new errors.IllegalArgumentError(configureErrorPrefix + 'options.refreshIdle: Must be a boolean!', { refreshIdle: { actual: typeof options.refreshIdle, expected: 'boolean' } });
		} else if (options.reapIntervalMillis && !utils.isNumber(options.reapIntervalMillis)) {
			throw new errors.IllegalArgumentError(configureErrorPrefix + 'options.reapIntervalMillis: Must be a number!', { reapIntervalMillis: { actual: typeof options.reapIntervalMillis, expected: 'number' } });
		} else if (options.priorityRange && !utils.isNumber(options.priorityRange)) {
			throw new errors.IllegalArgumentError(configureErrorPrefix + 'options.priorityRange: Must be a number!', { priorityRange: { actual: typeof options.priorityRange, expected: 'number' } });
		}
	}
	...
	function Connection(options) {
		...
		this.configure = function (options, strict, cb) {
			if (options === true) {
				...
				strict = true;
				options = {};
			} else if (utils.isFunction(strict)) {
				cb = strict;
				strict = false;
			}
			options = options || {};
			if (cb && !utils.isFunction(cb)) {
				throw new errors.IllegalArgumentError(configureErrorPrefix + 'cb: Must be a function!', { actual: typeof cb, expected: 'function' });
			}
			this.pending = Promise.resolve().bind(this)
				.then(function () {
					sanitizeConfigOptions(options);
					if (_pool) {
						console.log('draining old pool');
						return this.drain().then(this.destroyAllNow);
					}
				})
				.then(function () {
					if (strict) {
						_config = utils.merge(utils.merge({}, defaults), options);
					} else {
						_config = utils.merge(_config, options);
					}
					_pool = createPool(_config);
					bindMethods.call(this, _pool);
				})
				.finally(function () {
					this.pending = true;
				}).nodeify(cb);
		...
		this.configure(options, true).done();
	}
	...
	Connection.prototype.run = function (query, options, cb) {
		...
		return Promise.resolve().bind(this)
			...
			.then(function () {
				return this.acquire().then(function (connection) {
					conn = connection;

					var deferred = Promise.defer();

					query.run(connection, options, function (err, result) {
						if (err) {
							deferred.reject(err);
						} else {
