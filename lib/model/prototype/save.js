  ...
  function save(options, cb) {
    ...
    return Promise.resolve().bind(this)
      ...
      .then(function (Model) {
        ...
        if (this.isNew()) {
          return Model.connection.run(Model.r.table(Model.tableName).insert(this.attributes, { returnVals: true }));
        } else {
          return Model.connection.run(Model.r.table(Model.tableName).get(this.get(Model.idAttribute)).update(this.attributes, { returnVals: true }));
        }
      })
