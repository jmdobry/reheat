...
module.exports = function (container, r, Promise, utils, errors) {
	 ...
	 *
	 * - `{IllegalArgumentError}` - `err` - `null` if no error occurs.
	...
	function destroyOne(primaryKey, options, cb) {
		...
		return Promise.resolve().bind(this)
			.then(function () {
				...
				query = r.do(r.table(this.tableName).get(primaryKey), function () {
					var subQuery;
					...
					if (Model.softDelete) {
						...
						subQuery = r.table(Model.tableName).get(primaryKey).update(attrs);
					} else {
						subQuery = r.table(Model.tableName).get(primaryKey).delete();
					}
					if (options.with) {
							...
							utils.forOwn(Model.relations.hasMany, function (relation, modelName) {
								if (!models[modelName]) {
									...
									if (models[modelName].softDelete) {
										...
										subQuery = r.table(models[modelName].tableName).getAll(primaryKey, { index: relation.foreignKey }).update(attrs).and(subQuery);
									} else {
										subQuery = r.table(models[modelName].tableName).getAll(primaryKey, { index: relation.foreignKey }).delete().and(subQuery);
									}
								}
							});
						}
			...
			.then(function () {
				return this.connection.run(query, options);
			})
			.finally(function () {
