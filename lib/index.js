...
container.resolve(function (r, Promise, robocop, utils, errors, extend, Model, Collection, Connection, models, collections) {
				...
				if (!model.relations.indices[index]) {
					(function (m, i) {
						m.relations.indices[i] = Promise.resolve().bind(m)
							...
							.then(function () {
								return this.connection.run(r.branch(r.table(this.tableName).indexList().contains(i), null, r.table(this.tableName).indexCreate(i)));
							})
							.finally(function () {
	...
	reheat.defineModel = function (name, staticProps, protoProps) {
		if (!utils.isString(name)) {
			...
			// Ensure table exists
			models[name].tableReady = models[name].connection.run(r.branch(r.tableList().contains(models[name].tableName), null, r.tableCreate(models[name].tableName)))
				.finally(function () {
