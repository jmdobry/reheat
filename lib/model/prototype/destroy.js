...
module.exports = function (r, Promise, utils, errors, models) {
	...
	function destroy(options, cb) {
		return Promise.resolve().bind(this)
			...
			.then(function () {
				return this.beforeDestroy().return({ r: this.constructor.connection, Model: this.constructor });
			})
			.then(function (helper) {
				return helper.r.branch(
					helper.Model.softDelete,
					helper.r.branch(
						helper.Model.timestamps,
						helper.r.table(helper.Model.tableName)
							.get(this.get(helper.Model.idAttribute))
							.update(helper.r.expr(this.attributes).merge({ deleted: helper.r.now(), updated: helper.r.now() }), { return_vals: true }),
						helper.r.table(helper.Model.tableName)
							.get(this.get(helper.Model.idAttribute))
							.update(helper.r.expr(this.attributes).merge({ deleted: helper.r.now() }), { return_vals: true })
					),
					helper.r.table(helper.Model.tableName)
						.get(this.get(helper.Model.idAttribute))
						.delete({ return_vals: true })
				).run(options);
			})
