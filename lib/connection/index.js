...
var Connection = module.exports = function (options) {
	...
	function _configure(options, strict, cb) {
		...
		if (!utils.isObject(options)) {
			error = new errors.IllegalArgumentError(errorPrefix + 'options: Must be an object!');
		} else if (options.port && !utils.isNumber(options.port)) {
			error = new errors.IllegalArgumentError(errorPrefix + 'options.port: Must be a number!');
		} else if (options.db && !utils.isString(options.db)) {
			error = new errors.IllegalArgumentError(errorPrefix + 'options.db: Must be a string!');
		} else if (options.host && !utils.isString(options.host)) {
			error = new errors.IllegalArgumentError(errorPrefix + 'options.host: Must be a string!');
		} else if (options.authKey && !utils.isString(options.authKey)) {
			error = new errors.IllegalArgumentError(errorPrefix + 'options.authKey: Must be a string!');
		} else if (options.name && !utils.isString(options.name)) {
			error = new errors.IllegalArgumentError(errorPrefix + 'options.name: Must be a string!');
		} else if (options.max && !utils.isNumber(options.max)) {
			error = new errors.IllegalArgumentError(errorPrefix + 'options.max: Must be a number!');
		} else if (options.min && !utils.isNumber(options.min)) {
			error = new errors.IllegalArgumentError(errorPrefix + 'options.min: Must be a number!');
		} else if (options.idleTimeoutMillis && !utils.isNumber(options.idleTimeoutMillis)) {
			error = new errors.IllegalArgumentError(errorPrefix + 'options.idleTimeoutMillis: Must be a number!');
		} else if (options.log && !utils.isBoolean(options.log) && !utils.isFunction(options.log)) {
			error = new errors.IllegalArgumentError(errorPrefix + 'options.log: Must be a boolean or a function!');
		} else if (options.refreshIdle && !utils.isBoolean(options.refreshIdle)) {
			error = new errors.IllegalArgumentError(errorPrefix + 'options.refreshIdle: Must be a number!');
		} else if (options.reapIntervalMillis && !utils.isNumber(options.reapIntervalMillis)) {
			error = new errors.IllegalArgumentError(errorPrefix + 'options.reapIntervalMillis: Must be a number!');
		} else if (options.priorityRange && !utils.isNumber(options.priorityRange)) {
			error = new errors.IllegalArgumentError(errorPrefix + 'options.priorityRange: Must be a number!');
		} else if (cb && !utils.isFunction(cb)) {
			error = new errors.IllegalArgumentError(errorPrefix + 'cb: Must be a function!');
		}
...
Connection.prototype.run = function (query, options, cb) {
	...
	if (!utils.isFunction(cb)) {
		throw new errors.IllegalArgumentError('Connection#run(query[, options], cb): cb: Must be a function!');
	} else if (!query) {
		return cb(new errors.IllegalArgumentError('Connection#run(query[, options], cb): query: Required!'));
	} else if (!utils.isObject(options)) {
		return cb(new errors.IllegalArgumentError('Connection#run(query[, options], cb): options: Must be an object!'));
	}
