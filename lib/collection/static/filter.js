...
module.exports = function (container, r, Promise, utils, errors, extend) {
	...
	function _filter(predicate, options) {
		...
		if (_this.model) {
			...
			Model = _this;
			_this = Model.collection;
		} else {
			Model = _this;
			_this = extend.apply(Collection, [
				{},
				{
					model: Model,
			...
			collections[Model.modelName + 'Collection'] = _this;
			...
			return Model.connection.run(query, options).then(function (cursor) {
				...
				cursor.toArray(function (err, documents) {
					if (err) {
						...
						var Constructor = _this;
						deferred.resolve(new Constructor(documents));
					}
				});
