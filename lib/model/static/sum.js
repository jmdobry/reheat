...
/**
 * @method Model.sum
 * @desc Sum aggregator method. Works on a single field or in conjunction with `groupBy`.
 * @static
 ...
 * @param {function} cb Callback function. Signature: <code>cb(err, result)</code>.
 * @see http://rethinkdb.com/api/javascript/#sum
 */
module.exports = function sum(predicate, options, cb) {
	predicate = predicate || {};
	options = options || {};
	if (utils.isFunction(predicate)) {
		...
		throw new Error('Model.sum([predicate][, options], cb): cb: Must be a function!');
	} else if (!utils.isObject(predicate)) {
		cb(new Error('Model.sum([predicate][, options], cb): predicate: Must be an object!'));
	} else if (!utils.isObject(options)) {
		cb(new Error('Model.sum([predicate][, options], cb): options: Must be an object!'));
	} else if (!utils.isString(predicate.sum)) {
		cb(new Error('Model.sum([predicate][, options], cb): predicate.sum: Must be a string!'));
	}

	predicate.where = predicate.where || {};

	if (utils.isString(predicate.where)) {
		predicate.where = JSON.parse(utils.predicate.where);
	}
	...
	if (predicate.groupBy) {
		query = query.groupBy(predicate.groupBy, r.sum(predicate.sum));
	} else {
		query = query.sum(predicate.sum);
	}
