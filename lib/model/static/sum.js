...
module.exports = function sum(predicate, options, cb) {
	...
	if (isFunction(predicate)) {
		...
		throw new Error('Model.sum(predicate, options, cb): predicate: Must be an object!');
	} else if (!isObject(options)) {
		throw new Error('Model.sum(predicate, options, cb): options: Must be an object!');
	} else if (!isFunction(cb)) {
		throw new Error('Model.sum(predicate, options, cb): cb: Must be a function!');
	} else if (!isString(predicate.sum)) {
		throw new Error('Model.sum(predicate, options, cb): predicate.sum: Must be a string!');
	}
