  ...
  function findOne(primaryKey, options, cb) {
    ...
    return Promise.resolve().bind(this)
      .then(function () {
        ...
        query = Model.r.do(Model.r.table(Model.tableName).get(primaryKey), function (doc) {
          if (options.with) {
            ...
            if (Model.relations.hasMany) {
              utils.forOwn(Model.relations.hasMany, function (relation, modelName) {
                if (!models[modelName]) {
                  ...
                  merge[localField] = Model.r.table(models[modelName].tableName).getAll(primaryKey, { index: foreignKey }).coerceTo('ARRAY');
      ...
      .then(function () {
        if (!utils.isEmpty(merge)) {
          return Model.connection.run(Model.r.table(this.tableName).indexWait(), options);
        } else {
          return null;
        }
      })
