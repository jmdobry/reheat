 ...
 * @name Connection
 * @description
 * The Connection class is for connecting to instances of RethinkDB.
 *
 * Every Model needs an instance of Connection in order to operate.
 *
 * Example:
 *
 * ```js
 ...
 * pre-defined queries. A Connection is a wrapper around [r#connect](http://rethinkdb.com/api/javascript/#connect)
...
var Connection = module.exports = function (options) {
	...
	/**
	 * @doc method
	 * @id Connection.methods:configure
	 ...
	 */
	this.configure = function configure(options, strict, cb) {
		...
		if (!utils.isFunction(cb)) {
			...
			cb(new errors.IllegalArgumentError(errorPrefix + 'options: Must be an object!', { actual: typeof options, expected: 'object' }));
		} else if (options.port && !utils.isNumber(options.port)) {
			cb(new errors.IllegalArgumentError(errorPrefix + 'options.port: Must be a number!', { port: { actual: typeof options.port, expected: 'number' } }));
		} else if (options.db && !utils.isString(options.db)) {
			cb(new errors.IllegalArgumentError(errorPrefix + 'options.db: Must be a string!', { db: { actual: typeof options.db, expected: 'string' } }));
		} else if (options.host && !utils.isString(options.host)) {
			cb(new errors.IllegalArgumentError(errorPrefix + 'options.host: Must be a string!', { host: { actual: typeof options.host, expected: 'string' } }));
		} else if (options.authKey && !utils.isString(options.authKey)) {
			cb(new errors.IllegalArgumentError(errorPrefix + 'options.authKey: Must be a string!', { authKey: { actual: typeof options.authKey, expected: 'string' } }));
		} else if (options.name && !utils.isString(options.name)) {
			cb(new errors.IllegalArgumentError(errorPrefix + 'options.name: Must be a string!', { name: { actual: typeof options.name, expected: 'string' } }));
		} else if (options.max && !utils.isNumber(options.max)) {
			cb(new errors.IllegalArgumentError(errorPrefix + 'options.max: Must be a number!', { max: { actual: typeof options.max, expected: 'number' } }));
		} else if (options.min && !utils.isNumber(options.min)) {
			cb(new errors.IllegalArgumentError(errorPrefix + 'options.min: Must be a number!', { min: { actual: typeof options.min, expected: 'number' } }));
		} else if (options.idleTimeoutMillis && !utils.isNumber(options.idleTimeoutMillis)) {
			cb(new errors.IllegalArgumentError(errorPrefix + 'options.idleTimeoutMillis: Must be a number!', { idleTimeoutMillis: { actual: typeof options.idleTimeoutMillis, expected: 'number' } }));
		} else if (options.log && !utils.isBoolean(options.log) && !utils.isFunction(options.log)) {
			cb(new errors.IllegalArgumentError(errorPrefix + 'options.log: Must be a boolean or a function!', { log: { actual: typeof options.log, expected: 'boolean|function' } }));
		} else if (options.refreshIdle && !utils.isBoolean(options.refreshIdle)) {
			cb(new errors.IllegalArgumentError(errorPrefix + 'options.refreshIdle: Must be a boolean!', { refreshIdle: { actual: typeof options.refreshIdle, expected: 'boolean' } }));
		} else if (options.reapIntervalMillis && !utils.isNumber(options.reapIntervalMillis)) {
			cb(new errors.IllegalArgumentError(errorPrefix + 'options.reapIntervalMillis: Must be a number!', { reapIntervalMillis: { actual: typeof options.reapIntervalMillis, expected: 'number' } }));
		} else if (options.priorityRange && !utils.isNumber(options.priorityRange)) {
			cb(new errors.IllegalArgumentError(errorPrefix + 'options.priorityRange: Must be a number!', { priorityRange: { actual: typeof options.priorityRange, expected: 'number' } }));
		}
	 ...
	 */
	this.destroy = _pool.destroy;

	/**
	 * @doc method
	 * @id Connection.methods:acquire
	 ...
	 */
	this.acquire = _pool.acquire;

	/**
	 * @doc method
	 * @id Connection.methods:release
	 ...
	 */
	this.release = _pool.release;

	/**
	 * @doc method
	 * @id Connection.methods:drain
	 * @name drain([cb])
	 * @description
	 ...
	 */
	this.drain = _pool.drain;

	/**
	 * @doc method
	 * @id Connection.methods:destroyAllNow
	 ...
	 * @param {function=} cb Callback function. Signature: `cb()`
	 */
	this.destroyAllNow = _pool.destroyAllNow;

	/**
	 * @doc method
	 * @id Connection.methods:pooled
	 ...
	 */
	this.pooled = _pool.pooled;

	/**
	 * @doc method
	 * @id Connection.methods:getPoolSize
	 * @name getPoolSize( )
	 * @description
	 ...
	 */
	this.getPoolSize = _pool.getPoolSize;

	/**
	 * @doc method
	 * @id Connection.methods:getName
	 * @name getName( )
	 * @description
	 ...
	 */
	this.getName = _pool.getName;

	/**
	 * @doc method
	 * @id Connection.methods:availableObjectsCount
	 ...
	 */
	this.availableObjectsCount = _pool.availableObjectsCount;

	/**
	 * @doc method
	 * @id Connection.methods:waitingClientsCount
	 ...
	 */
	this.waitingClientsCount = _pool.waitingClientsCount;
};

/**
 * @doc method
 * @id Connection.methods:run
